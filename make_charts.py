...
SUBSCRIBE_KEYWORDS = {
    ...
    "세탁기구독":    {"삼성": ["삼성 세탁기 렌탈","비스포크 세탁기 렌탈","삼성 세탁기 구독"],
                    "LG":   ["LG 세탁기 렌탈","트롬 세탁기 렌탈","LG 세탁기 구독"]},
    "식기세척기구독": {"삼성": ["삼성 식기세척기 렌탈","비스포크 식기세척기 렌탈","삼성 식기세척기 구독"],
                    "LG":   ["LG 식기세척기 렌탈","디오스 식기세척기 렌탈","LG 식기세척기 구독"]},
    "의류관리기구독": {"삼성": ["에어드레서 렌탈","삼성 에어드레서 렌탈","에어드레서 구독"],
                    "LG":   ["스타일러 렌탈","LG 스타일러 렌탈","스타일러 구독"]},
    "청소기구독":    {"삼성": ["삼성 청소기 렌탈","비스포크 제트 렌탈","삼성 청소기 구독"],
                    "LG":   ["LG 청소기 렌탈","코드제로 렌탈","LG 청소기 구독"]},
    "로봇청소기구독": {"삼성": ["삼성 로봇청소기 렌탈","비스포크 로봇청소기 렌탈","삼성 로봇청소기 구독"],
...
.gap-badge{{
    ...
    border-radius:8px;padding:3px 10px 3px 7px;font-size:11px;font-weight:700;
    border:1.5px solid;
}}
...
function buildGapAnnotations(cat, seriesSlice, labels, filterLabel) {{
    ...
    const n = s.length;
    const out = {{ gapEnd: anno(labels[n-1], s[n-1], l[n-1]) }};
    if (n > 1) out.gapStart = anno(labels[0], s[0], l[0]);

    if (n > 1 && filterLabel) {{
        const delta = calcGap(s[n-1], l[n-1]) - calcGap(s[0], l[0]);
        const deltaColor = delta >= 0 ? '#1428A0' : '#A50034';
        const tri = delta >= 0 ? '▲' : '▼';
        out.gapDelta = {{
            type: 'label',
            xValue: labels[n-1],
            yValue: Math.max(s[n-1], l[n-1]),
            content: `${{filterLabel}}比 ${{tri}} ${{Math.abs(delta).toFixed(1)}}%p`,
            backgroundColor: deltaColor + 'ee',
            color: '#fff',
...
function updateGapBadge(cid, cat, seriesSlice) {{
    ...
    const n = s.length;
    const gapPctStart = l[0] !== 0 ? (s[0] - l[0]) / l[0] * 100 : 0;
    const gapPctEnd   = l[n-1] !== 0 ? (s[n-1] - l[n-1]) / l[n-1] * 100 : 0;
    if (Math.abs(gapPctEnd) > Math.abs(gapPctStart)) {{
        const diff = (Math.abs(gapPctEnd) - Math.abs(gapPctStart)).toFixed(1);
        const color = gapPctEnd >= 0 ? '#1428A0' : '#A50034';
        el.querySelector('.gap-txt').textContent = '갭확대 +' + diff + '%p';
        el.querySelector('.tri').style.borderBottomColor = color;
        el.style.color = color;
        el.style.borderColor = color + '59';
